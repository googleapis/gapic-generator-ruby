      ...
      module ServiceYamlParser
        class << self
            ...
            def parse_service_yaml service_yaml_text
                ...
                $stderr.puts ("-"*7)+"yaml"+("-"*7)
                $stderr.puts service_yaml
                $stderr.puts ("-"*20)
            ...
            def parse_apis apis_yaml
                $stderr.puts ("-"*7)+"apis"+("-"*7)
                $stderr.puts apis_yaml
                $stderr.puts ("-"*20)
            ...
            def parse_http http_yaml
                $stderr.puts ("-"*7)+"http"+("-"*7)
                $stderr.puts http_yaml
                $stderr.puts ("-"*20)

                http = Google::Api::Http.new

                if http_yaml.key RULES_KEY
                    http.rules = http_yaml[RULES_KEY].map { |rule_yaml| parse_http_rule rule_yaml  }
                end
