...
module Grafeas
  module V1
    ...
    #     Note: The following parameters are mutually exclusive: `provenance`, `slsa_provenance`, `slsa_provenance_zero_two`. At most one of these parameters can be set. If more than one is set, only one will be used, and it is not defined which one.
    # @!attribute [rw] slsa_provenance
    #   @return [::Grafeas::V1::SlsaProvenance]
    #     Note: The following parameters are mutually exclusive: `slsa_provenance`, `provenance`, `slsa_provenance_zero_two`. At most one of these parameters can be set. If more than one is set, only one will be used, and it is not defined which one.
    # @!attribute [rw] slsa_provenance_zero_two
    #   @return [::Grafeas::V1::SlsaProvenanceZeroTwo]
    #     Note: The following parameters are mutually exclusive: `slsa_provenance_zero_two`, `provenance`, `slsa_provenance`. At most one of these parameters can be set. If more than one is set, only one will be used, and it is not defined which one.
