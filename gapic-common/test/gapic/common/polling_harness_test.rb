...
class PollingHarnessTest < Minitest::Test
  ...
  def test_wait_perform_delay_once
    ...
    retry_policy_mock.expect :validate_policy!, nil, []
    retry_policy_mock.expect :reset_elapsed_time!, 0, []
    retry_policy_mock.expect :perform_delay!, true, []
    retry_policy_mock.expect :retry_with_timeout?, true, []
  ...
  def test_wait_with_timeout
    ...
    polling_harness = Gapic::Common::PollingHarness.new retry_policy: retry_policy
    wait_count = 0
    result = polling_harness.wait do
      if wait_count > 2
        return "wait_with_timeout_result"
      end
      wait_count += 1
      nil
    end
