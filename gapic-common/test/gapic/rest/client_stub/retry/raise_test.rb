...
class ClientStubRetryRaiseTest < Minitest::Test
  ...
  def test_no_retry_without_codes
    ...
    make_request_proc = lambda do |args|
      call_count += 1
      raise FakeError.new(GRPC::Core::StatusCodes::INTERNAL)
    end
    ...
    ::Gapic::Rest::ClientStub.stub :base_make_http_request, make_request_proc do
      ...
      refute_nil ex.cause
      assert_kind_of FakeError, ex.cause
  ...
  def test_times_outs
    ...
    make_request_proc = lambda do |args|
      call_count += 1
      raise FakeError.new(GRPC::Core::StatusCodes::UNAVAILABLE)
    end
